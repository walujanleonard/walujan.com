...
const CompetitionCertificateList: React.FC = () => {
  const CompetitionCertificateListsDesc =
    CompetitionCertificateLists.length > 1 &&
    CompetitionCertificateLists[0].year > CompetitionCertificateLists[1].year
      ? CompetitionCertificateLists
      : [...CompetitionCertificateLists].reverse();

  return (
    <>
      {CompetitionCertificateListsDesc.map((yearData, index) => (
        <div key={index}>
          <div className="mt-10">
            <h2>{yearData.year}</h2>
            <ul>
...
export default CompetitionCertificateList;
