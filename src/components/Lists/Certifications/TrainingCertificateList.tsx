import { TrainingCertificateLists } from "@/data/certifications/TrainingCertificateLists";
...
const TrainingCertificateList: React.FC = () => {
  const TrainingCertificateListsDesc =
    TrainingCertificateLists.length > 1 &&
    TrainingCertificateLists[0].year > TrainingCertificateLists[1].year
      ? TrainingCertificateLists
      : [...TrainingCertificateLists].reverse();

  return (
    <>
      {TrainingCertificateListsDesc.map((yearData, index) => (
        <div key={index}>
          <div className="mt-10">
            <h2>{yearData.year}</h2>
            <ul>
